...
if (-not (Test-Path -LiteralPath $projectRules)) {
    Copy-Item (Join-Path $sourceRoot 'template/AGENTS.md') $projectRules
}

$config = Join-Path $opencode 'sayrhazi.yaml'
if (-not (Test-Path -LiteralPath $config)) {
    Copy-Item (Join-Path $sourceRoot 'template/sayrhazi.yaml') $config
}

$opencodeJson = Join-Path $opencode 'opencode.json'
if (-not (Test-Path -LiteralPath $opencodeJson)) {
    Copy-Item (Join-Path $sourceRoot 'template/opencode.json') $opencodeJson
}

# Le watcher est contrôlé par Sayrhazi : il est mis à jour avec -Force.
$watcherSource = Join-Path $sourceRoot 'scripts/watch-work.py'
