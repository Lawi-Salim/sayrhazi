...
def main() -> int:
    ...
    try:
        ...
        return 2
    if not (core / "template" / "sayrhazi.yaml").is_file():
        print("ERREUR : noyau Sayrhazi introuvable (template/sayrhazi.yaml absent).")
        return 2
    ...
    latest = read_version(core / "template" / "sayrhazi.yaml") or "(inconnue)"
    ...
    for src in sorted((core / "agents").glob("*.md")):
        ...
        updated += 1
    watcher = core / "scripts" / "watch-work.py"
    if watcher.is_file():
    ...
    added, backup, mwarns = migrate_config(opencode / "sayrhazi.yaml", core / "template" / "sayrhazi.yaml", opencode / "history")
    if backup:
    ...
    core_names = {p.name for p in (core / "agents").glob("*.md")}
